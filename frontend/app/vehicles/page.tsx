...
const availablesFilters = ['Name', 'Model', 'Vehicle_Class', 'Manufacturer'];
...
export default function VehiclesList() {
    ...
    async function fetchVehicles(page: string) {
        try {
            ...
            const url = page ? page : `${process.env.NEXT_PUBLIC_API_URL}/vehicles?${queryString}`;
    ...
    function handlePageChange(page: number | string) {
        if (typeof page === 'number') {
            ...
            const queryString = new URLSearchParams(filteredFilters).toString();
            const url = queryString.includes('page') ? `${process.env.NEXT_PUBLIC_API_URL}/vehicles?${queryString}` : `${process.env.NEXT_PUBLIC_API_URL}/vehicles?page=${page}${queryString}`;
            setCurrentPage(page);
    ...
    function clearFilters(event: React.MouseEvent<HTMLButtonElement>) {
        ...
        const url = `${process.env.NEXT_PUBLIC_API_URL}/vehicles?page=${currentPage}`;
        fetchVehicles(url);
    }
        ...
        <div className="md:h-[90vh] flex flex-col">
            ...
            <main className="flex-grow flex flex-col md:flex-row items-start">
                    ...
                    <form onSubmit={handleSubmit} className={`flex gap-2 flex-col ${isMainMenuOpen ? 'flex' : 'hidden'} md:flex`}>
                        ...
                        {/* <div className='flex gap-2'>
                            ...
                            >
                                ...
                                {}
                                {
                                    filters.cost_in_credits && (sortOrder === -1 ? 'ASC' : 'DESC')
                                }
                            </button>
                        </div> */}
                        <div className='flex gap-2'>
                ...
                <section className="flex-grow p-4">
                    <h2 className="p-4 font-bold text-3xl text-title">Vehicles: </h2>
                                    ...
                                    vehicles.map((item, index) => (
                                        <div key={index} className="bg-card text-cardForeground p-4 rounded-lg shadow min-h-20 transition-all duration-300 hover:scale-105 hover:shadow-lg">
                                            ...
                                            <div className="text-md flex gap-2">
                                                ...
                                                <p>{item.vehicle_class.charAt(0).toUpperCase() + item.vehicle_class.slice(1)}</p>
                                            </div>
                                            <div className="text-md flex gap-2">
                                            ...
                                            <Link
                                                href={`/vehicles/${item.url.split('/')[item.url.split('/').length - 2]}`} // divide la url en secciones y obtenemos el numero que esta entre los / / finales
